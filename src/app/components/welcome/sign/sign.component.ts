...
export class SignComponent implements OnInit {
  ...
  step1X(){//CREATE NEW USER
    if(this.typeRedirect !== 'POS' && !this.auth.refrCode || this.typeRedirect !== 'POS' && this.auth.refrCode.length !== 6){
      this.auth.resource.startSnackBar("Please enter a valid Refr code.");
    }else{

      if(this.typeRedirect !== 'POS'){

      
      this.auth.checkIfCodeExist(this.auth.refrCode).then(ref => {
          console.log("checkIfCodeExist", ref)
        if(!ref || !ref.success || !ref.exists){
          this.auth.resource.startSnackBar("No such Refr code exist.")
        }else{
...
*/
  ...
  goToDash(){
    console.log("MEGAMAN 11")
    this.auth.user$.pipe(take(1)).subscribe((mine:any) => {
      if(mine){
        if(!this.typeRedirect){
          console.log("MEGAMAN 12")
          ...
          console.log("MEGAMAN 13")

          if(this.typeRedirect == "laterF2F" || this.typeRedirect == "nowF2F"){
            this.auth.addUserToCode(this.auth.refrCode, mine.uid).then(() => {
    
              if(this.typeRedirect == "laterF2F"){
            this.auth.resource.router.navigate(['/sync-contacts']);
              }
              if(this.typeRedirect == "nowF2F" && this.data.code){
                this.auth.resource.router.navigate(['/go/' + this.data.code ]);
              }
          ...
          if(
            this.typeRedirect == "POS" || this.typeRedirect == "B2U" || this.typeRedirect == "B2P" //|| this.typeRedirect == "F2U"
          ){
            console.log("I AM RAM")

          setTimeout(() => {
            const includesSavedStore = !mine.storeSav ? false : mine.storeSav?.includes( this.data.sid ) ;
            this.dialogRef.close({success:true, uid: mine.uid, name:mine.name, type:this.typeRedirect, includesSavedStore: includesSavedStore});
          }, 500)
